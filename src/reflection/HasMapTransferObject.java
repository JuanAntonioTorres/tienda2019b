...
public class HasMapTransferObject {
        ...
        for (int i = 0; i < metodosDeclarados.length; i++) {
            for (int j = 0; j < atributos.length; j++) {
                if (metodosDeclarados[i].getName().toLowerCase().contains("set" + atributos[j].getName().toLowerCase())) {
                    ejecutarSet(datosFila.get(atributos[j].getName().toLowerCase()), genericObject, metodosDeclarados[i]);
                }
            }
        }
    ...
    private void ejecutarSet(Object dato, Object genericObject, Method metodosDeclarado) throws IllegalAccessException, InvocationTargetException {
